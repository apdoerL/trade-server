...
@Component
@Slf4j
public class OpenPosiHandleThreadPool implements BackPressure {


    @Value("${thread.posi-open.core-pool-size:20}")
    private Integer corePoolSize;

    @Value("${thread.posi-open.max-pool-size:20}")
    private Integer maxPoolSize;

    @Value("${thread.posi-open.keep-alive_time:30}")
    private Long keepAliveTime;

    @Value("${thread.posi-open.queue-capacity:300000}")
    private Integer queueCapacity;

    @Value("${thread.posi-open.queue-backpressure:280000}")
    ...
    public void init() {
        ...
        log.info(" open posi  thread pool init success core:{},max:{},keepAlive:{},initQueue:{},backPressure:{}", corePoolSize, maxPoolSize, keepAliveTime, queueCapacity, queueBackPressure);
    }
